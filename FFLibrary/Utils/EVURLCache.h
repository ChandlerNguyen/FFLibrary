...
/**
 * Danh sách các request được filter/replace, mỗi phần tử của danh sách có kiểu là NSDictionary
 * Ví dụ: thay các request đến từ ad360.vn bằng chuỗi rỗng
 * {
 *     @"pattern" : @"ad360.vn",
 *     @"type"  : "String",
 *     @"value" : @""
 * }
 * Hoặc thay các ảnh png bằng ảnh myimage.png
 * {
 *     @"pattern" : @".png",
 *     @"type"  : "Image",
 *     @"value" : @"myimage.png"
 * }
 */
@property (nonatomic, strong) NSArray *substitutionPaths;

/// Danh sách các request được bỏ qua, không thực hiện caching/filtering
@property (nonatomic, strong) NSArray *exclusionPaths;
@property (nonatomic, assign) BOOL isDisableWebFilterFeature;    // YES - Bỏ qua chức năng filtering/caching
...
+ (EVURLCache *)instance;   // Nang added
